      ...
      def self.extract_updated_code(analysis, original_content = nil, verbose = false)
        ...
        if analysis =~ /```ruby\s*\n(.*?)\n```/m
          ...
          if original_content && extracted_code
            ...
            if (orig_module_count > 0 && ext_module_count == 0) || 
               (orig_class_count > 0 && ext_class_count == 0) ||
               (orig_end_count > 0 && ext_end_count == 0)
              ...
              return extracted_code if !extracted_code.empty?
            end
          else
        ...
        if all_code_blocks.size > 1
          ...
          if combined_code.include?("module") && combined_code.include?("end")
        ...
        if original_content && analysis.include?('ISSUE:')
          ...
          if analysis =~ /exit[^!]/i && analysis =~ /exit!/i
            modified_content = original_content.gsub(/\bexit\s+(\d+)/) { "exit! #{$1}" }
        ...
        analysis.scan(/```(?:ruby)?\s*\n(.*?)\n```/m).flatten.each do |block|
          if block && !block.empty?
            extracted_code = block if !extracted_code || block.length > extracted_code.length
          end
        end
        ...
        if original_content && (!extracted_code || extracted_code.length < original_content.length * 0.5)
          ...
          puts "Code extraction incomplete (#{extracted_code&.length || 0} vs #{original_content.length} bytes), using original content." if verbose
          return original_content
        end
